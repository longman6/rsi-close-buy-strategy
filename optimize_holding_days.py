"""
전략 B (RSI 5, SMA 50) 보유일수 최적화
- 다양한 최대 보유일수 설정으로 백테스트
- 각 설정별 성과 비교
"""
...
# 테스트할 최대 보유일수 목록
HOLDING_DAYS_TO_TEST = [10, 20, 30, 40, 50, 60, 70, 90, 999]  # 999 = 무제한
...
# 시뮬레이션 엔진 (보유일수 제한 추가)
# ---------------------------------------------------------
def run_simulation_with_holding_limit(stock_data, valid_tickers, max_holding_days):
    """
    Run simulation with maximum holding days limit.

    매도 조건:
    1. RSI > SELL_THRESHOLD (70)
    2. 보유일수 >= max_holding_days
    """
    ...
    return final_ret, mdd, win_rate, len(trades_df), avg_holding, avg_win, avg_loss
...
def run_optimization():
    print("=" * 70)
    print("전략 B (RSI 5, SMA 50) 보유일수 최적화")
    print("=" * 70)
    ...
    for max_days in HOLDING_DAYS_TO_TEST:
        label = f"{max_days}일" if max_days < 999 else "무제한"
        print(f"\n테스트 중: 최대 보유일수 = {label}")

        ret, mdd, win_rate, trades, avg_hold, avg_win, avg_loss = run_simulation_with_holding_limit(
            stock_data, valid_tickers, max_days
        )
        ...
        print(f"  거래: {trades}회, 평균보유: {avg_hold:.1f}일")
    ...
    # 파일 저장
    output_file = "reports/holding_days_optimization.md"
    ...
    # CSV 저장
    csv_file = "reports/holding_days_optimization.csv"
...
def generate_optimization_report(results_df):
    ...
    report = f"""# 전략 B (RSI 5, SMA 50) 보유일수 최적화 분석
...
테스트한 최대 보유일수: {', '.join([f"{d}일" if d < 999 else "무제한" for d in HOLDING_DAYS_TO_TEST])}
...
"""
    ...
    for _, row in results_df.iterrows():
        ...
        report += f"| {row['AvgWin']:.2f}% | {row['AvgLoss']:.2f}% | {sharpe:.2f} |\n"
...
"""
    ...
    # 보유일수 구간별 분석
    short_term = results_df[results_df['MaxDays'] <= 30]
    mid_term = results_df[(results_df['MaxDays'] > 30) & (results_df['MaxDays'] <= 60)]
    long_term = results_df[results_df['MaxDays'] > 60]

    if not short_term.empty:
        report += f"**단기 보유 (≤30일):**\n"
        report += f"- 평균 수익률: {short_term['Return'].mean():.2f}%\n"
        report += f"- 평균 승률: {short_term['WinRate'].mean():.2f}%\n"
        report += f"- 평균 거래 횟수: {short_term['Trades'].mean():.0f}회\n\n"

    if not mid_term.empty:
        report += f"**중기 보유 (31-60일):**\n"
        report += f"- 평균 수익률: {mid_term['Return'].mean():.2f}%\n"
        report += f"- 평균 승률: {mid_term['WinRate'].mean():.2f}%\n"
        report += f"- 평균 거래 횟수: {mid_term['Trades'].mean():.0f}회\n\n"

    if not long_term.empty:
        report += f"**장기 보유 (>60일):**\n"
        report += f"- 평균 수익률: {long_term['Return'].mean():.2f}%\n"
        report += f"- 평균 승률: {long_term['WinRate'].mean():.2f}%\n"
        report += f"- 평균 거래 횟수: {long_term['Trades'].mean():.0f}회\n\n"
...
"""
    ...
    for i, (_, row) in enumerate(top_3_return.iterrows(), 1):
        report += f"{i}. {row['Label']}: 수익률 {row['Return']:.2f}%, 승률 {row['WinRate']:.2f}%\n"
