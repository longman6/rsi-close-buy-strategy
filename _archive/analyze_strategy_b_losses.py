...
def generate_loss_report(top_100, losses):
    ...
    report = f"""# 전략 B (RSI 5, SMA 50) 손실 거래 분석
...
"""
    ...
    # 3.1 연도별 분포
    report += "### 3.1 연도별 손실 거래 분포 (상위 100개)\n\n"
    year_dist = top_100['SellYear'].value_counts().sort_index()
    report += "| 연도 | 거래 수 | 비율 |\n| :--- | ---: | ---: |\n"
    for year, count in year_dist.items():
        report += f"| {year} | {count}개 | {count/len(top_100)*100:.1f}% |\n"

    # 3.2 보유 기간 분석
    report += "\n### 3.2 보유 기간 분석 (상위 100개)\n\n"
    report += f"- **평균 보유 기간:** {top_100['HoldingDays'].mean():.1f}일\n"
    report += f"- **최소 보유 기간:** {top_100['HoldingDays'].min()}일\n"
    report += f"- **최대 보유 기간:** {top_100['HoldingDays'].max()}일\n"
    report += f"- **중앙값:** {top_100['HoldingDays'].median():.1f}일\n\n"

    # 보유 기간 구간별 분포
    bins = [0, 10, 20, 30, 40, 50, 100, 200, 999]
    labels = ['0-10일', '11-20일', '21-30일', '31-40일', '41-50일', '51-100일', '101-200일', '200일+']
    ...
    for range_name, count in holding_dist.items():
        report += f"| {range_name} | {count}개 | {count/len(top_100)*100:.1f}% |\n"

    # 3.3 손실률 분포
    report += "\n### 3.3 손실률 분포 (상위 100개)\n\n"
    report += f"- **평균 손실률:** {top_100['PnL_Pct'].mean():.2f}%\n"
    report += f"- **최대 손실률:** {top_100['PnL_Pct'].min():.2f}%\n"
    report += f"- **중앙값:** {top_100['PnL_Pct'].median():.2f}%\n\n"
    ...
    report += f"- **평균 매수 RSI:** {top_100['BuyRSI'].mean():.2f}\n"
    report += f"- **최소 매수 RSI:** {top_100['BuyRSI'].min():.2f}\n"
    report += f"- **최대 매수 RSI:** {top_100['BuyRSI'].max():.2f}\n"
    report += f"- **중앙값:** {top_100['BuyRSI'].median():.2f}\n\n"
    ...
    if len(repeat_losses) > 0:
        ...
        for ticker, count in repeat_losses.items():
            ...
            report += f"| {name} | {ticker} | {count}회 | {total_loss:,.0f}원 |\n"
    else:
        report += "반복 손실 종목 없음\n"
    ...
    if avg_holding > 30:
        report += f"2. **보유 기간 특징:** 평균 {avg_holding:.1f}일로, 비교적 긴 보유 기간 후 손실\n"
    else:
        report += f"2. **보유 기간 특징:** 평균 {avg_holding:.1f}일로, 빠른 손절\n"

    # 손실률 크기
    avg_loss_pct = top_100['PnL_Pct'].mean()
    if avg_loss_pct < -20:
        report += f"3. **손실률 특징:** 평균 {avg_loss_pct:.2f}%로, 큰 폭의 손실 발생\n"
    else:
        report += f"3. **손실률 특징:** 평균 {avg_loss_pct:.2f}%로, 비교적 제한적인 손실\n"

    # RSI 매수 시점
    avg_buy_rsi = top_100['BuyRSI'].mean()
    report += f"4. **매수 시점 RSI:** 평균 {avg_buy_rsi:.2f} (기준: 35 미만)\n"

    # 반복 손실 종목
    if len(repeat_losses) > 0:
        report += f"5. **반복 손실:** {len(repeat_losses)}개 종목이 2회 이상 손실 기록\n"
    else:
        report += "5. **반복 손실:** 없음 (각 종목은 다른 시기에 손실)\n"
...
def main():
    print("=" * 60)
    print("전략 B (RSI 5, SMA 50) 손실 거래 분석")
    print("=" * 60)
