...
def run_simulation_optimized(stock_data, valid_tickers, 
    ...
    # Note: stock_data already has SMA and RSI based on initial call args (RSI 3, SMA 200).
    # IF we vary RSI or SMA, we must re-calculate columns dynamically.
    
    # Since we are varying SMA, we need to recalc SMA column.
    # RSI is fixed at 3, so that's fine.
    
    for ticker, df in stock_data.items():
        daily_close = df['Close']
        df['SMA_Dynamic'] = daily_close.rolling(window=sma_window).mean()
        # Ensure RSI 3 is accurate (it should be from prepare_data call)
        # We assume prepare_data was called with rsi_window=3.
...
def run_optimization():
    ...
    # Pre-load data with max requirements (RSI 3, SMA 200)
    stock_data, valid_tickers = prepare_data_batch(tickers, START_DATE)
    
    # Grid Search Space (Dense)
    sma_periods = [20, 50, 60, 100, 120, 200]
    buy_thresholds = [5, 10, 15, 20, 25, 30, 35]
    ...
    print(f"\n🔍 Starting Dense Optimization for RSI 3... ({total_tests} combinations)")
    ...
    # Save to CSV
    csv_file = "rsi3_optimization_results.csv"
...
"""
    ...
    with open("rsi3_optimization_report.md", "w") as f:
        f.write(md_report)
